...
class OPN(nn.Module):
    ...
    def read(self, mkey, mval, mhol, frame, valid, dist): 
        ...
        # get 1/4 scale mask
        peel3 = (F.interpolate(peel, size=(int(height/4), int(width/4)), mode='bilinear', align_corners=False) >= 0.5)
        ...
        qpel = peel3
        # read.
        read = self.MaskedRead(qkey, qval, qpel, mkey, mval, (1-mhol.float()).bool())
        # decode
